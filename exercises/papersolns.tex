...
\prob{3}  The upwind scheme for the SKE is in the slides, and we assume $a_j=0$ and $w_j=0$.

Assuming non-negative horizontal velocity $u_j\ge 0$, the scheme is:
...
The new surface value is a strict average of the current surface elevation pair $s_{j-1},s_j$.

A similar result holds for rightward flow.  Assuming CFL $|u_j|\Delta t \le \Delta x$, for $u_j<0$ one gets $\snew_j = c_0 s_j + c_+ s_{j+1}$ as a strict average with $c_0\ge 0$, $c_+\ge 0$, and $c_0+c_+=1$.

Thus, in this $a=0$ and $w=0$ case, the upwind method is stable \emph{if one also uses time steps $\Delta t$ which satisfy CFL}.  This is because unstable modal growth is impossible, for if the current surface is any wave of any amplitude, the updated surface may still be wavy but its amplitude will be reduced because all the bumps are averaged-out.
...
\prob{5}  Recall that $H=s-b$.  In what follows we will assume:
\begin{itemize}
\item the SKE holds,
...
We start with the flux term in the mass continuity equation, expand the definition of $\bar u H$, then apply the Leibniz rule (\textbf{4} above), and then apply the basal condition on $u$:
\begin{align*}
...
\prob{6}  Starting from the 5 boxed equations on slide 24, the formulas for $w(z)$, $p(z)$, and $\tau_{13}(z)$ on slide 25 are straightforward to get by integration, using the given boundary conditions.
...
\medskip
\noindent \emph{Comment.}  Before trusting this approximation of the solution of the power-law Stokes equations, observe that one does not actually get the basal sliding function $u_0(t,x)$ as data!  Rather, one needs to solve stress balances \emph{both} for deformation within the ice (e.g.~$\partial u/\partial z$) \emph{and} for the basal sliding velocity $u_0$.  In this context, however, there is no way for the SIA simplified model to balance the longitudinal (or ``membrane'') stresses which control the actualy basal sliding velocity $u_0$.  For this purpose, 21st-century practitioners recommend that the sliding be determined by solving a non-trivial stress balance.  That is, one should either use a hybrid balance combining the SIA and the shallow shelf approximation, or a unified balance like the Blatter-Pattyn equations, or even the (full) Stokes model.  In glaciology there is no physically-based model for sliding which is as cheap as the SIA is for internal deformation.


\prob{7}  The basic point, when one is thinking about how the non-sliding SIA model converts a given glacier shape into a surface velocity field, is that ice flows downhill in this model.  That is, the SIA model always makes the horizontal component point opposite the surface gradient: $u \sim - \partial s/\partial x$.

The next point is that the magnitude of the horizontal surface velocity $|u|$ scales with the 4th power\footnote{Assuming $n=3$ as usual.} of the ice thickness.  For a given surface slope, relatively small increases in thickness correspond to substantial increases in surface speed.

Next is something that is not at all obvious from any of the equations in the slides.  Namely, how does the vertical component $w$ respond to surface slope and thickness in the non-sliding SIA model?  \emph{Answer:}  It acts diffusively.

...
def bedelevation(xy):
    return (
        600.0 - 300.0 * (L - xy[1] + xy[0]) / L + 200.0 * ((L - xy[1] - xy[0]) / L) ** 2
    )
...
F = epsreg * inner(grad(s), grad(v)) * dx + (-inner(u, ns) - a) * v * dx
...
nvs = NonlinearVariationalSolver(problem, solver_parameters=sp)

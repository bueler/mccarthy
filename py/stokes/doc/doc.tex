...
\begin{center}
\footnotesize
\emph{version: \today~for McCarthy 2026}
\end{center}
...
\end{equation}
so equation \eqref{forcebalance} simply says $-\Div \sigma = \rho \bg$.  One may derive from \eqref{cauchystress} that $p = -\frac{1}{3} \trace \sigma$ in 3D, thus the pressure is the negative of the average normal stress.  By definition $\Div\tau$ in \eqref{forcebalance} is a vector with components which are the divergences of the columns:
\begin{equation}
    \left(\nabla \cdot \tau\right)_i = \left(\tau_{1i}\right)_{x_1} + \left(\tau_{2i}\right)_{x_2} + \left(\tau_{3i}\right)_{x_3}  \label{divtaudefn}
\end{equation}
...
\end{align*}
($|\tau|^2$ is also known as the \emph{second invariant} of the stress tensor.)  Using \eqref{viscflowlaw} we can eliminate $\tau$ from equation \eqref{forcebalance}, thereby rewriting the system in terms of velocity $\bu$ and pressure $p$ only:
\begin{align}
- \nabla \cdot \left(B_n |D\bu|^{\frac{1}{n} - 1} D\bu\right) + \nabla p &= \rho \mathbf{g} \label{stokes} \\
\Div \bu &= 0 \label{incompagain}
\end{align}

From now on we consider a 2D region $\Omega$, and we denote the coordinates by $(x,z)$.  We add an angle parameter: gravity is at an angle $\alpha$ with the $z$-direction, so that $\bg = \left<g\sin\alpha,-g\cos\alpha\right>$ where $g>0$.  The numerical model in these notes solves system \eqref{stokes}, \eqref{incompagain} with a body force given by $\rho \bg$, and the problem solution is a pair $\bu,p$.
...
\left(B_n |D\bu|^{\frac{1}{n} - 1} D\bu - pI\right) \hbn &= - \rho g (H_{\text{out}}-z)\left<\cos\alpha, \sin\alpha \right> && \text{\emph{outflow}} \label{outflowbc}
\end{align}
...
Equations \eqref{stokes} and \eqref{incompagain} are called the \emph{strong form} of the Glen-Stokes model.  An integral equation form of the same equations, the \emph{weak form}, is needed when building a numerical finite element (FE) method \cite{Elmanetal2014}.  This new form will be derived by multiplying the strong form equations by test functions and then integrating over $\Omega$, which defines a nonlinear functional $F$, that is, a function which takes functions as input and returns scalars.  The weak form says that $F$ is zero when acting on all of the test functions.  The weak form has two well-known advantages:
\begin{itemize}
\item It has a larger space of possible solutions than the strong form, for instance it is more flexible with respect to discontinuities in the data.
\item An FE method can create test functions via local constructions on meshes of triangles or quadrilaterals.  These piecewise-defined functions are allowed in the weak form.  Such a framework is more flexible with respect to problem geometry than finite difference methods based on the strong form.
\end{itemize}

A solution to the weak form below is a velocity-pressure pair $\bu,p$.  Each function lives in a certain function space.  We write $\bu\in V_D$ and $p \in Q$.  (The Sobolev spaces $V_D$ and $Q$ are precisely-identified in \cite{JouvetRappaz2011}.)  Test functions $\bv\in V_0$ and $q\in Q$ come from nearly the same spaces, with $V_D$ and $V_0$ differing only by the value on the Dirichlet boundary: $\bu\in V_D$ satisfies a nonhomogeneous inflow condition \eqref{inflowbc} while $\bv\in V_0$ is zero there.
...
\end{equation}
However, the actual definition of $F$ re-balances the number of derivatives on $\bv,q$ versus $\bu,p$, to have only first derivatives on $\bu,\bv$, and no derivative on $p,q$.  For this we need integration-by-parts.

Recall the product rule $\nabla \cdot(f\bX) = \grad f\cdot \bX + f \nabla \cdot \bX$ and the divergence theorem $\int_\Omega \nabla \cdot (f\bX) = \int_{\partial \Omega} f\bX \cdot \hbn$.  Denoting $\tau = B_n |D\bu|^{\frac{1}{n} - 1} D\bu$ for convenience we have, in 2D,
\begin{align*}
...
By linearity of $F$ in its last two arguments, to solve \eqref{feweak} it suffices to consider only the hat function basis of the test function spaces $V_0^h$ and $Q^h$.  Note that relative to \eqref{weak} the nonlinear functional $F$ is unchanged and we have merely added ``$h$'' superscripts to all continuum quantities, thus problem \eqref{feweak} can be shown to be well-posed by the same theory that applies to the continuum problem \cite[Theorem 4.3]{JouvetRappaz2011}.

FE problem \eqref{feweak} is a nonlinear system which should be solved by Newton's method \cite{Bueler2021,Kelley2003}.  To sketch this, suppose we have some current iterate $\bu^h,p^h$ which does not exactly solve \eqref{feweak}.  Then for each test function $\bv^h$ or $q^h$, i.e.~for each hat function, the value of the ``residual'' $F(\bu^h,p^h;\bv^h,q^h)$ can be computed concretely, though also approximately, by quadrature \cite{Bueler2021,Elmanetal2014}.  Also we can use the current iterate, and the derivatives of $F$, to evaluate the Jacobian at the current iterate.  This ``assembly'' process\footnote{Actually the equations are assembled element-by-element, not node-by-node.  For each triangle $\triangle_k$ the contribution to \eqref{feweak} is added to the correct discrete equation.  However, Firedrake  \cite{Rathgeberetal2016} hides the assembly process so that we may think about the problem at a higher level.} generates a system of sparse, linear algebraic equations, namely the Newton step equations of the discrete Glen-Stokes problem.  Each Newton step requires us to solve a sparse and indefinite linear system.  These systems have a well-known block structure (Figure \ref{fig:lowspy}), and we may either apply a direct solver for the whole matrix or use a preconditioned Krylov iteration \cite{Elmanetal2014,GolubVanLoan2013}.  Because solver technology is complicated we abandon further explanation, but see Chapter 14 of \cite{Bueler2021} for both strategies on the linear Stokes equations.
...
\section{Implementation with Python and Firedrake} \label{sec:implementation}

This section gives a practical view of our codes and how they are used in practice.  In overview, we implement problem \eqref{feweak}, and make the $P_1$/$P_2$ discretization choices, in Python using Firedrake \cite{Rathgeberetal2016}.  The PETSc \cite{Balayetal2018,Bueler2021} and Mumps \cite{Amestoy2001} solver libraries are called via Firedrake on the discretized nonlinear equations.

To define the nonlinear residual function $F$ in \eqref{defineF} in code, we use the UFL domain-specific language, designed for describing such weak forms \cite{Alnaesetal2014}.  This language comes rather close to the mathematical form.  For example, compare \eqref{strainrate} to the following:
\begin{Verbatim}[fontsize=\small,frame=lines]
def D(U):
    return 0.5 * (grad(U) + grad(U).T)
\end{Verbatim}
The flow law \eqref{viscflowlaw} and the weak form \eqref{defineF} are implemented as follows in \texttt{momentummodel.py}:
\begin{Verbatim}[fontsize=\small,frame=lines]
...
\end{Verbatim}
Note that we have regularized the flow law with a typical strain rate scale \texttt{Dtyp} and a dimensionless parameter $\eps=$ \texttt{eps} $>0$.  Details of the body force and outflow boundary condition are in the same code.

Firedrake knows how to take these expressions, plus a choice of mesh and function spaces, and run the assemble process to create the Newton step equations for this problem.  Then Firedrake's \texttt{solve()} command solves \eqref{feweak} \cite{Rathgeberetal2016}.

As shown in Figure \ref{fig:blockdiagram}, our solver is factored into three major Python components:
\begin{itemize}
\item \texttt{domain.py}: \quad  This writes an outline of the domain $\Omega$ into an ASCII file (\texttt{.geo} extension) using the Gmsh \cite{GeuzaineRemacle2009} geometry description language.  Gmsh can then be used to mesh this domain at the command-line (below).  Portions of the boundary are marked with integer tags; see the Python dictionary \texttt{bdryids}.
...
\begin{tikzpicture}[node distance=3.8cm,>=latex]
    ...
    \node [module] (momentummodel)[above left of=flow, yshift=-8mm] {\begin{minipage}{4.0cm} \texttt{momentummodel.py} \footnotesize (\texttt{class MomentumModel}) \end{minipage}};
...
\medskip
\caption{Users interact with the tools in order.}
...
(firedrake) $ paraview output/result.pvd       # visualize results
\end{Verbatim}
%$

\medskip
\noindent Paraview can be used to generate Figure \ref{fig:glacier:bedstep} from the output file.  The \verb|README.md| file documents the usage more thoroughly, including mesh refinement and parallel solutions.

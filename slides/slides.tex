...
FIXME add $\bn_s$ arrow to figure
\end{center}
...
\begin{itemize}
\item here $\bu=(u,w)$ denotes the velocity \alert{at the ice surface}
\item $\bn_s$ is a vector which points upward and is normal (perpendicular) to the ice surface: \quad $\displaystyle \bn_s = \left(-\frac{\partial s}{\partial x}, \,1\right)$
...
\end{equation*}
    ...
    \begin{itemize}
    \item[$\circ$] assume $a,u,w$ are given functions/values \comm{fixed in hour 2}
    \end{itemize}
\item \alert{numerical modeling goal:} use a computer program to \emph{approximately} evolve the glacier surface $z=s(t,x)$ according to the SKE
    \begin{itemize}
...
\begin{itemize}
\item now the SKE\, $\left(\frac{\partial s}{\partial t} + u \frac{\partial s}{\partial x} = a + w\right)$\, becomes discrete:
    $$\frac{\snew_j - s_j}{\Delta t} + u_j \frac{s_{j+1} - s_{j-1}}{2\Delta x} = a_j + w_j$$
\item we can write this as an equation which determines $\snew_j$:
...
then bad (unstable) things happen! \comm{exercise?}
...
{\footnotesize
    ...
    \item[$\circ$] separate functions define $a(x)$, $u(x)$, $w(x)$
    \item[$\circ$] \texttt{x} is equally-spaced
    \item[$\circ$] all indices except zero: \, $j=$\texttt{ 1:}
...
surface travels at speed $u$
\item from this $(t,x)$ grid picture, the

upwind scheme can only be

stable if
$$|u_j| \Delta t \le \Delta x \hspace{45mm}$$
...
\item<2-> most of your work is to understand the math, not to write the code
...
\item<4-> respect surface mass/energy balance just as much as ice dynamics
\item<5-> machine learning \emph{after} continuum mechanics
\end{itemize}
\end{frame}
...
\begin{itemize}
\item answer: glacier surfaces are generally smoother than the bedrock topography, which means that $H$ is rougher than $s$
\end{itemize}
\end{frame}

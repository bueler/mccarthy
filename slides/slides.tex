...
\begin{frame}{plan for hour 1: numerical glacier shape modeling using SKE}
...
\end{equation*}
\item \alert{numerical goal:} use a computer program to track of the surface of the glacier $z=s(t,x)$ under some assumptions
    \begin{itemize}
    \item[$\circ$] grid of points $\{x_j\}$
    \item[$\circ$] unknowns are the surface elevations on the grid $s_j(t) \approx s(t,x_j)$
    \item[$\circ$] assume $a,u,w$ are given functions/values \uncover<2>{\hfill \alert{$\leftarrow$ this is crazy!}}
    \item[$\circ$] short Python program
    \end{itemize}
\item \alert{problems and issues:}
     ...
     \item practical: implementation, debugging, visualization?
     \end{enumerate}
\end{itemize}
\end{frame}
...
{\footnotesize
    \begin{itemize}
    \item[$\circ$] \texttt{s} and \texttt{x} are NumPy arrays
    \item[$\circ$] \texttt{x} is equally-spaced
    \item[$\circ$] \texttt{[1:]} is all indices except the first, and \texttt{[:-1]} is all except the last

...
\item CFD textbooks are of limited utility to glaciologists
...
\item ice fluid is \emph{slow} and \emph{non-Newtonian}
    ...
    \item[$\circ$] ice is non-Newtonian in a ``shear-thinning'' way
        \begin{itemize}
        \item ``non-Newtonian'' means viscosity $\nu$ is not constant, but instead depends on the fluid state
...
\end{align*}
\item in the standard model, the velocity and pressure fields of a glacier are \alert{instantaneous functions} of the glacier geometry and its boundary stresses
...
\begin{itemize}
\item add known (but non-penetrating) basal velocity conditions:
	$$u(0)=u_0, \qquad w(0)=0$$
\item add zero normal stress condition at surface $z=H=s$:
	$$p(H)=0$$
...
\vspace{-2mm}
\item note $\tau_{13}$ is linear in depth
...
with positive coefficient $\Gamma$, so \alert{ice flows downhill}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item notational convenience: given $j$, let
\begin{align*}
...
\begin{itemize}
\item computing the surface value of the horizontal velocity at each regular grid location $x_j$ is a Python function:
\begin{lstlisting}[language=PythonPlus]
def u(x, b, s):
...
\begin{itemize}
\item the equations of the \alert{time-dependent SIA model}, using surface elevation as the variable, are the following:
\begin{align*}
\frac{\partial s}{\partial t} + u \frac{\partial s}{\partial x} &= a + w \\
u &= - \frac{2}{n+1} A (\rho g)^n \left|\frac{\partial s}{\partial x}\right|^{n-1} \frac{\partial s}{\partial x} \left[\big(s - b\big)^{n+1} - \big(s-z\big)^{n+1}\right] \\
...
\item[] \alert{mathematical issues}
    \begin{enumerate}\setcounter{enumi}{2}
    \item well-posedness not fully resolved \comm{talk separately?}
    \end{enumerate}
\item[] \alert{numerical questions}
...
\begin{itemize}
\item \alert{time-dependent SKE and Glen-law Stokes model}:
\begin{align*}
...
D\mathbf{u}_{ij} &= A \tau^{n-1} \tau_{ij}
\end{align*}
\item implementing well is a major topic of current research
  \begin{itemize}
  \item[$\circ$] subject (as always) to \alert{$s \ge b$}
  \item[$\circ$] I do not trust \emph{anyone's} view of time stepping (yet)
  \end{itemize}
%\item projects 13, 14 look at numerical solutions of the Stokes equations only (w/o SKE)
\end{itemize}
\end{frame}
...
\item[] \alert{modeling questions}
    ...
    \end{enumerate}
\item[] \alert{mathematical question}
    \begin{enumerate}\setcounter{enumi}{2}
    \item well-posedness of SKE+Stokes?
    \end{enumerate}
\item[] \alert{numerical questions/recommendations/issues}
    \begin{enumerate}\setcounter{enumi}{3}
    ...
    \item as a practical matter, use a powerful finite element \cite{Elmanetal2014} library
        \begin{itemize}
        \item[$\circ$] Firedrake? Elmer? FENiCs? Moose? \dots
        \end{itemize}
    \item solver performance is a major concern \dots\, I'm sure the speed will be adequate by 2050
    \end{enumerate}
\end{itemize}
\end{frame}
...
\item<3-> writing your own numerical models is of great value, but you should toss the result in the trash once you understand it
    ...
    \end{itemize}
\item<4-> surface mass/energy balances are as important as ice dynamics

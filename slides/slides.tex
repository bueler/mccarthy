...
\bigskip
\item for liquid water one uses the incompressible Navier-Stokes equations:
\begin{align*}
...
\begin{itemize}
\item nonetheless, numerical glacier modelling is first-class ``computational fluid dynamics''
  \begin{itemize}
...
\bigskip
\item thus the standard model is \alert{(Glen-Steinmann power law) Stokes}:
\begin{align*}
\nabla \cdot \mathbf{u} &= 0 &&\text{\emph{incompressibility}} \\
0 &= - \nabla p + \nabla \cdot \tau_{ij} + \rho\, \mathbf{g} &&\text{\emph{stress balance}} \\
...
\end{align*}
    ...
    \begin{itemize}
    \item[$\circ$] in fact the standard model of glaciology is:

\alert{SKE $+$ (Glen-law) Stokes}
    \end{itemize}
...
\end{align*}
\item thus a time-stepping ice sheet code can (must?) recompute the velocity field at every time step
  \begin{itemize}
  \item[$\circ$] but velocity from the previous time step is \emph{not} required
...
\item $n=3$ Glen-law Stokes equations say:
\begin{empheq}[]{align}
...
\begin{align*}
\mathbf{g} &= g \sin\alpha\, \hat x - g \cos \alpha \,\hat z \hspace{40mm} \\
...
\begin{itemize}
\item now add boundary conditions:
...
\medskip
\item from $u_z = 2 A \tau^2 \tau_{13}$ get a \alert{velocity formula}:
\vspace{-0.05in}
\begin{align*}

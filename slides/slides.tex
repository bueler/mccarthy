...
\begin{itemize}
\item \alert{model:} the shallow ice approximation (SIA) adopts the above slab-on-slope formula for $u(x)$, but as a general formula for glaciers
  \begin{itemize}
...
\item \alert{numerics:}
  \begin{itemize}
  \item[$\circ$] finite difference computation of $u(x)$ from SIA
  \item[$\circ$] finite difference computation of $w(x)$ from SIA and incompressibility \comm{(less trivial)}
  \end{itemize}
\item \alert{to address:}
     \begin{enumerate}
...
\begin{itemize}
\item the Glen-law, planar, non-sliding shallow ice approximation (SIA) is the following system of equations:
\begin{align*}
...
\end{align*}
  ...
  \begin{itemize}
  \item[$\circ$] $u,w$ depend on $x$ and $z$
...
\begin{itemize}
\item differencing the surface elevations gives \alert{slopes computed on the staggered grid}:
...
\end{align*}
\item then \alert{horizontal velocity is defined at all levels}:
...
\begin{itemize}
\item we integrate vertically to recover the surface value of $w$ from $\partial u/\partial x$, by \alert{using incompressibility}:
\begin{align*}
w(x,s(x)) &= w(x,s(x)) - w(x,b(x)) \\
   &= - \int_{b(x)}^{s(x)} - \frac{\partial u}{\partial x}(x,z)\,dx
\end{align*}
\item thus get regular-grid value $w_j \approx w(x_j,s(x_j))$ by integrating vertically:
\begin{align*}
...
\begin{center}
FIXME %\includegraphics[width=0.7\textwidth]{steady}
\end{center}
...
\begin{frame}{time-dependent SKE approximation}

\begin{itemize}
\item FIXME: show
\item implementing is an exercise
\end{itemize}
\end{frame}

...
\frac{\partial s}{\partial t} + u \frac{\partial s}{\partial x} = a + w,
\end{equation*}
let us approximate partial derivatives by finite difference\footnote{helpful textbooks include LeVeque \cite{Leveque2007} and Morton \& Mayers \cite{MortonMayers2005}} quotients:
...
\item instead, it is known that when the ``advecting velocity'', here $u_j$, is positive then the ``upwind'' version is stable:
    ...
    \begin{itemize}
    \item[$\circ$] when $u_j<0$ use $\partial s/\partial x \approx (s_{j+1}-s_j)/\Delta x$
    \item[$\circ$] actually, it is ``conditionally stable''
    \end{itemize}
\item actually, this scheme is ``conditionally stable''
    \begin{itemize}
    \item[$\circ$] the condition is known \comm{demo soon}
    \end{itemize}
...
{\footnotesize
    ...
    \item[$\circ$] separate functions define $a(x)$, $u(x)$, $w(x)$ \comm{made-up values here!}
...
\medskip
\item versus time-dependent SKE in 2D: \quad $\displaystyle \frac{\partial s}{\partial t} - \bu \cdot \bn_s = a$
\end{itemize}
\end{frame}

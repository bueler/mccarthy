...
\vspace{-2mm}
\includegraphics[width=0.4\textwidth]{liquid} \hfill \uncover<2>{\includegraphics[width=0.35\textwidth]{lighterfluidalpha}}
\end{frame}
...
\item thus a time-stepping ice sheet code recomputes the full velocity field at every time step
  ...
  \end{itemize}
\item because there is no memory of it, velocity is a ``diagnostic'' output not needed for starting or restarting the model
\end{itemize}
\end{frame}

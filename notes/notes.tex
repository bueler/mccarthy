...
\end{align}
The velocity field $\bu$ is divergence-free (equation \eqref{incompressible}), thus no fluid is created or lost within the domain.  Equation \eqref{navierstokes} says ``$ma=F$''; it is Newton's second law.  The left side has the acceleration $\mathbf{u}_t + \mathbf{u}\cdot\nabla \mathbf{u}$, while the right-hand side is the total force per unit volume, the sum of viscous, pressure, and gravity forces, respectively.  If $\sigma$ denotes the Cauchy stress tensor, the right side could also be written $\nabla \cdot \sigma + \rho \mathbf{g}$.  Note that incompressibility \eqref{incompressible} is a constraint on the possible solutions of \eqref{navierstokes}. 

The space of solutions to \eqref{incompressible} and \eqref{navierstokes} is rich.  It includes turbulent flows, which we will not need here.  In fact, reading a book-length introduction to fluids such as \cite{Acheson1990}, or to their numerical solution as in \emph{computational fluid dynamics} (CFD), for example \cite{Wesseling2001}, is overkill for glacier modeling, but perhaps recommended for broader education.  One might even ask: is ice flow modelling really a part of CFD?  Glacier flow is indeed a large-scale fluid problem, comparable to atmosphere and ocean circulation, but on the other hand the topics which make such flows exciting, for example turbulence, convection, coriolis force, and density stratification, are irelevant to ice flow.  What could be interesting about the flow of slow and strong ice?

\subsection*{The Glen-Stokes equations}  We have sais that glacier ice is a ``slow'' fluid.  In terms of equation \eqref{navierstokes}, this is precisely means that $\rho \left(\mathbf{u}_t + \mathbf{u}\cdot\nabla \mathbf{u}\right) \approx 0$, in other words that the forces (stresses) of inertia are negligible, so that Newton's second law becomes a \emph{stress balance}.  However, ice is also a shear-thinning fluid with a specific kind of nonlinearly-viscous (``non-Newtonian'') behavior in which larger strain rates imply smaller viscosity.  The viscosity $\nu$ in \eqref{navierstokes} is therefore not constant; one must add an empirically-based flow law.  Combining these ideas gives the canonical slow-and-shear-thinning ice flow model:
\begin{align}
...
(D\bu)_{ij} &= A \tau^2 \tau_{ij} &&\text{\emph{$n$=3 Glen flow law}} \label{flowlaw}
\end{align}
...
Because the Stokes equations do not contain any time derivatives, the combined effect of glacier geometry, boundary stresses, gravity $\rho \mathbf{g} = \left<0,0,-\rho g\right>$, and a value for ice softness $A>0$ will determine the velocity and stress fields ($\bu$, $p$, $\tau_{ij}$) \cite{JouvetRappaz2011}.  In fact, equations \eqref{incompressibleagain}--\eqref{flowlaw} apply at each instant, even as the glacier geometry, the basal stresses, and thus the solution fields all change in time.\footnote{Ice flow simulation codes therefore need no memory of prior momentum or velocity.  Said another way, velocity is a ``diagnostic'' output of ice flow models.  Unlike in a Navier-Stokes-based simulation, velocity is not needed for (re)starting a simulation.}
...
\end{align}
Here $\tau_{13}$ is a shear stress while $\tau_{11},\tau_{33}$ are deviatoric longitudinal stresses.  In this planar case note that $\tau_{33}=-\tau_{11}$, by incompressibility and the flow law, which explains the form of \eqref{stokespz}, and also a short calculation shows $\tau^2 = \tau_{11}^2+\tau_{13}^2$.  Equations \eqref{incompressiblexz}--\eqref{forceflowz} now form a system of five nonlinear equations in five scalar unknowns ($u,w,p,\tau_{11},\tau_{13}$).

\subsection*{Slab-on-a-slope}  However, equations \eqref{incompressiblexz}--\eqref{forceflowz} are complicated enough!  Before jumping into numerical solution methods we consider a simplified situation, namely a uniform slab of ice.  In this simple case we can exactly solve the Stokes equations, and the same calculation motivates the shallow model in the next subsection.
...
\subsection*{Forms of the flow law}  Flow law \eqref{flowlaw} has another form.  Recall $\tau^2 = (1/2) \tau_{ij} \tau_{ij}$; also define $|D\bu|^2 = (1/2) (D\bu)_{ij} (D\bu)_{ij}$.  The scalars $\tau$ and $|D\bu|$ are tensor \emph{norms}.  From \eqref{flowlaw} we get $|D\bu| = A \tau^3$ so $\tau = A^{-1/3} |D\bu|^{1/3}$.  Thus \eqref{flowlaw} can be rewritten
\begin{equation}
\tau_{ij} = 2 \nu\, (D\bu)_{ij}  \qquad \text{\emph{viscosity form of a flow law}} \label{viscosityflowlaw}
\end{equation}
where for the Glen $n=2$ case we define
\begin{equation}
\nu = \frac{1}{2} A^{-1/3} |D\bu|^{-2/3}, \label{viscositypowerlaw}
\end{equation}
the nonlinear \emph{viscosity} $\nu$.  Often $B = A^{-1/3}$ is called the \emph{ice hardness}.  Equation \eqref{viscosityflowlaw} allows us to eliminate stresses $\tau_{ij}$ from the Stokes equations, resulting in formulas using only on the strain rates $(D\bu)_{ij}$.

\subsection*{The hydrostatic and Blatter-Pattyn approximation}  Instead of the severe simplifications of the slab-on-a-slope model, one may approximate the plane-flow Stokes equations \eqref{incompressiblexz}--\eqref{forceflowz} by assuming that horizontal variation in the vertical shear stress is small compared to the other terms.  This removes the single term  $\tau_{13,x}$ from the $z$-component of the stress balance \eqref{stokespz}, thus
\begin{equation}
\tau_{11,z} + p_z = - \rho g. \label{hydrostaticpz}
\end{equation}
Because $\sigma_{ij} = \tau_{ij} - p \delta_{ij}$, so $\tau_{11}+p = - \tau_{33}+p = - \sigma_{33}$, equation \eqref{hydrostaticpz} says that the vertical normal stress $\sigma_{33}$ is linear in depth.\footnote{A slightly-different model assumes that the pressure $p$ itself is hydrostatic.}  Taking a surface value of zero we get
\begin{equation}
\tau_{11}+p = \rho g (H-z). \label{hydrostaticitself}
\end{equation}

Equation \eqref{hydrostaticitself} allows elimination of the pressure $p$ from the model equations \eqref{incompressiblexz}--\eqref{forceflowz}.  In fact, taking the $x$-derivative of \eqref{hydrostaticitself}, then substituting into \eqref{stokespx} to eliminate $p_x$, and then using the viscosity form \eqref{viscosityflowlaw} leads to this equation \cite{GreveBlatter2009}:
\begin{equation}
...
Note that $p$ is gone from the simplified model formed by equations \eqref{incompressiblexz}, \eqref{viscositypowerlaw}, \eqref{stresshydrostatic}, but the hydrostatic stress balance remains non-trivially coupled to incompressibility because the vertical velocity $w$ appears in both places.  If we additionally drop $w_x$ from equation \eqref{stresshydrostatic} then we get the even simpler Blatter-Pattyn model:
\begin{equation}
\left(4 \nu u_x\right)_x + \left(\nu u_z\right)_z = \rho g H_x \qquad\text{\emph{Blatter-Pattyn stress balance}} \label{stressblatter}
\end{equation}
Solving \eqref{viscositypowerlaw} and \eqref{stressblatter} as a boundary value problem yields the scalar horizontal velocity $u$, and then one may recover $w$ from incompressibility \eqref{incompressiblexz} (and integration in $z$).  Because it has a form similar to the well-understood Poisson equation $u_{xx} + u_{zz} = f$, e.g.~as in \cite{LeVeque2007,MortonMayers2005}, the process of solving equation \eqref{stressblatter} may be more familiar than \eqref{stresshydrostatic}.


\section{Surface kinematical equation, and the evolution of glaciers}   \label{sec:masscont}

However, the Glen-Stokes model for glaciers, and its above approximations such as the Blatter-Pattyn model, are woefully incomplete in one critical sense.  They describe the ice flow at every instant but they give no clue how glaciers \emph{change}.  The Glen-Stokes equations have no time derivatives.
...
\begin{equation*}
\text{SKE:} \qquad \frac{\partial s}{\partial t} = a + \bu \cdot {\color{PineGreen} \bn_s}
\end{equation*}

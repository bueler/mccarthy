...
This scope includes the surface kinematical equation in one and two horizontal dimensions, and the shallow ice approximation in one horizontal dimension.  (These technical terms will be defined shortly.)  In fact we will also start with the equations of the non-shallow Stokes model, and we will adopt a constructive approach:
  \begin{quote}\emph{we provide understandable Python codes that do particular jobs.}\end{quote}

\subsection*{Notation}  Our notation, which generally follows references \cite{GreveBlatter2009,SchoofHewitt2013}, is in Table \ref{tab:notation}.  Cartesian coordinates are $x,y,z$ and time $t$, with $z$ positive-upward.  When these coordinates appear as subscripts they denote partial derivatives: $u_x = \partial u/\partial x$.  Tensor notation uses subscripts from the list $\{1,2,3,i,j\}$.  Thus $\tau_{ij}$ and $\tau_{13}$ denote entries of the deviatoric stress tensor.  We will also use a few abbreviations (Table \ref{tab:abbrev}).
...
The space of solutions to \eqref{incompressible} and \eqref{navierstokes} is rich.  It includes turbulent flows, which we will not need here, and in fact reading a book-length introduction to fluids such as \cite{Acheson1990}, or their numerical solution (\emph{computational fluid dynamics}) such as \cite{Wesseling2001}, is overkill for glacier modeling.  (Perhaps recommended for broader education?)  One might even ask: is ice flow modelling really computational fluid dynamics at all?  Glacier flow is a large-scale, climate-relevant fluid problem, comparable like ocean circulation for example.  On the other hand the topics which make such flows exciting, for example turbulence, convection, coriolis force, and density stratification, are irrelevant to ice flow.  What could be interesting about the flow of slow and strong ice?
...
\subsection*{Time-dependent ice domain}  The Glen-Stokes stress balance model for ice flow, and its approximations just described, are incomplete as models for glacier behavior in one critical sense.  They describe the ice flow at every instant but they give no clue how glaciers \emph{change}.  (Recall that the stress balance equations contain no time derivatives.)  In fact, the stress balance equations do not directly ``see'' the climate in which the glacier lives.\footnote{The stress balance ``sees'' basal stress as a boundary condition.  This is sometimes regarded as a climate input, but of course that involves additional models.}

However, the \emph{domain} on which the stress balance equations are solved evolves in time.  We need notation for this time-dependent glacier geometry.  Let $s(t,x,y)$ be the evolving ice surface elevation; the ice surface itself is $z=s(t,x,y)$ in 3D.  We assume that the glacier bed is a time-independent\footnote{Erosion and earth deformation (e.g.~glacial isostasy) are two processes which would make the bed elevation time dependent, in which case the \emph{basal kinematical equation} \eqref{basekine} is needed.} surface $z=b(x,y)$.  The stress balance differential equations, whichever they are, are solved on the 3D set
\begin{equation}
\left\{(x,y,z)\,:\,b(x,y) < z < s(t,x,y)\right\}. \label{domain}
\end{equation}
As show in Figure \ref{fig:domain}, in the planar (2D) case this notation becomes $z=s(t,x)$, $z=b(x)$, and $\left\{(x,z)\,:\,b(x) < z < s(t,x)\right\}$ (labeled ``\textsf{ice}''), respectively.  In other words, an important feature of glacier equations is that the ice domain is what evolves in time.  The instantaneous stress balance equations are solved on this evolving domain.

\onefig{domain}{Notation for the evolving geometry of a planar (2D) glacier.}

\subsection*{Surface mass balance and surface kinematics}  The upper surface evolves in part through the \emph{surface mass balance rate} (SMB), denoted $a(t,x,y)$, or $a(t,x)$ in the planar case.  The SMB is also known as the \emph{climatic} mass balance \cite{Cogleyetal2011}.  By definition this is the balance between accumulating snow and the loss of melt water, by melt of the snow, firn, or ice, through runoff, at the upper surface.\footnote{These notes only consider simulations of land-based glaciers, without floating portions, but ice shelves are also subject to a \emph{basal} mass balance rate.}  Our elevations are measured in meters, and time in seconds, and so the SMB function $a$ is measured in ice-equivalent meters per second.  Finally, note that ``SMB'' is one of a few abbreviations used in these notes; see Table \ref{tab:abbrev}.

The upper surface evolves according to the \emph{surface kinematical equation} (SKE; \cite{GreveBlatter2009,SchoofHewitt2013}).  In the planar case it is the equation
\begin{equation}
\frac{\partial s}{\partial t} = a + w -u \frac{\partial s}{\partial x}. \label{skeplanar}
\end{equation}
The planar ice velocity is $\bu=\left<u,w\right>$ in components, and the variables $u,w$ in \eqref{skeplanar} are the upper surface values, namely $u=u(t,x,s(t,x))$ and $w=w(t,x,s(t,x))$, because $z=s(t,x)$.  Equation \eqref{skeplanar} says that the ice surface moves up and down, at the rate $\partial s/\partial t$, according to the three things on the right side, each defined at the surface of the ice.  Namely, the SMB $a$, the vertical ice velocity $w$, and the product of horizontal ice velocity $u$ with the surface slope $\partial s/\partial x$.  Dimensionally, at least, this is correct; all terms are in meters per second.

\begin{table}[t]
\caption{Abbreviations.}
\begin{tabular}{cll}
abbreviation & meaning & precise definition \\ \hline
CFL & stability criterion in \cite{CFL1928} & inequality FIXME \\
...
\end{equation}
where $\bu=\left<u,v,w\right>$ is evaluated at the surface.  By moving all uses of the unknown $s$ to the left, form \eqref{ske} of the SKE suggests we might think of the climate $a$ as given data on the right, and we might solve it for that unknown.

\subsection*{First numerical glacier model}  About equation \eqref{skeplanar} or \eqref{ske} one might say:
\begin{center}
\emph{by numerically solving this equation one has a model for a glacier.}
\end{center}
However, what we really mean is that this equation is \emph{correct} in a physical sense and that it can be \emph{understood} as a glacier model.  As a model it is very incomplete, but in this section we will numerically solve the planar case \eqref{skeplanar} of the SKE.

To do this we make the bold assumption that the SMB function $a(t,x)$, \emph{and} the surface values of the velocity components, namely $u(t,x)$ and $w(t,x)$, are given by other agents.  In practice the SMB could be the output of a sub-model, such as a mass/energy process model coupled to atmosphere physics and data, and this is highly-nontrivial.  Likewise, the surface velocity components $u,w$ could be the output of a stress balance model as in section \ref{sec:continuum}, which then depends on the surface elevation $s$ itself to determine the ice domain geometry for the stress balance!  In fact the whole problem is non-trivially coupled together \cite{SchoofHewitt2013}, but we have to start somewhere in getting our numerical hands dirty.

So we are going to build a short Python program, \href{https://github.com/bueler/mccarthy/blob/master/py/surface1d.py}{\texttt{py/surface1d.py}} in the download (section \ref{sec:intro}), which evolves the glacier surface $z=s(t,x)$ according to \eqref{skeplanar}.  It generates an approximation to $s(t,x)$ over space $0<x<L$ and time $0<t<T$ intervals, that is, a \emph{time-dependent numerical solution} for the surface elevation.  We'll use this first program to consider questions of accuracy and stability, and then address practicalities like debugging, verification, and visualization.

The program is based on the \emph{finite difference} (FD) idea \cite{LeVeque2007, MortonMayers2005}.  Recall from calculus that Taylor's theorem says that for a smooth function $f(x)$ and a real number $\Delta$,
	$$f(x+\Delta) = f(x) + f'(x) \Delta + \frac{1}{2} f''(x) \Delta^2 + \frac{1}{3!} f'''(x) \Delta^3 + \dots$$
(The precise theorem states a finite sum and supplies an estimate of the remainder, but our loose version will suffice here.)  One may replace ``$\Delta$'' by its negative,
    $$f(x-\Delta) = f(x) - f'(x) \Delta + \frac{1}{2} f''(x) \Delta^2 - \frac{1}{3!} f'''(x) \Delta^3 + \dots,$$
or by other multiples, to get related forms.

FD methods for differential equations are constructed by combining such expressions to approximate the derivatives in an equation.  The resulting algebraic equations relate unknown gridded values via mere arithmetic.  As a simple example, note that if we take the above expression for $f(x+\Delta)$, and subtract $f(x)$ and then divide by $\Delta$, this yields an approximation of the first derivative:
...
We want to apply this FD approach to a \emph{partial} differential equation, so choose a time step $\Delta t>0$ and a grid spacing $\Delta x>0$.  We'll apply Taylor expansions for each variable to approximate the partial derivatives appearing in the SKE:
\begin{align*}
\frac{\partial s}{\partial t}(t,x) &= \frac{s(t+\Delta t,x) - s(t,x)}{\Delta t} + O(\Delta t), \\
\frac{\partial s}{\partial x}(t,x) &= \frac{s(t,x+\Delta x) - s(t,x-\Delta x)}{2\Delta x} + O((\Delta x)^2).
\end{align*}

Actually we want to be a bit more careful and distinguish between the solution $s(t,x)$ of the precise, continuum SKE, versus our numerical approximation.

FIXME need grid stencil cartoon
...
the SKE becomes discrete:
    $$\frac{\snew_j - s_j}{\Delta t} + u_j \frac{s_{j+1} - s_{j-1}}{2\Delta x} = a_j + w_j$$
write as an equation which determines $\snew_j$:
...
upwinding instead, it is known that an \emph{upwind} scheme is stable

conditionally stable, anyway, for this kind of \emph{advection} problem

namely, when the velocity is positive ($u_j \ge 0$) then:
	$$\hspace{3mm} \snew_j = s_j - \Delta t\, u_j \frac{s_j-s_{j-1}}{\Delta x} + \Delta t (a_j + w_j) \hfill \hspace{1.2cm} \leftarrow \text{\emph{useful}}$$

general upwind formula: \quad $\displaystyle \frac{\partial s}{\partial x} \approx \frac{1}{\Delta x} \begin{cases} (s_j-s_{j-1}), & u_j \ge 0 \\ (s_{j+1}-s_j), & u_j < 0 \end{cases}$

implementation

this upwind scheme for the SKE (assumes $u_j \ge 0$)

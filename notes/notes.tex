...
\subsection*{Numerical solution of generalized diffusion equations}  We are trying to model flowing ice not heat conduction!  In this section, as a step toward an SIA code which works on nonflat beds, \texttt{diffusion.m} below is an implemented numerical scheme for a generalized diffusion equation:
\begin{equation}
...
For numerical stability and mass conservation we compute $D$ on a staggered grid, as above in \texttt{diffusion.m}.  Various schemes for staggered-grid diffusivity have been proposed, but all involve averaging $H$ and differencing $h$ in a ``balanced'' way onto the staggered grid.  In the code \texttt{siaflat.m} below, which only works for a flat bed and zero surface mass balance, we use the Mahaffy \cite{Mahaffy} method, with the stencil for computing $D$ shown in Figure \ref{fig:diffstencil} (right).  The deficiences of \texttt{siaflat.m} are corrected in an extended code \texttt{siageneral.m} (not shown).
...
Program \texttt{siaflat.m} calls \texttt{diffusion.m}, so we already have a complicated code.  How do we make sure that such an implemented numerical scheme is correct?  Here are three proposed techniques:
\begin{enumerate}
...
FIXME the numbers below are not quite what I get with Octave; to test also with Matlab I need to fix the \texttt{diffusion.m} shadow issue by renaming to \texttt{diffstag.m}
...
\exer{5}{Show that when written as a formula for $T_j^{n+1}$, implicit scheme \eqref{implicit1D} has only positive coefficients.  Using \cite{MortonMayers} etc., as needed, explain why this shows it is unconditionally stable.}

\exer{6}{\emph{This multi-part exercise concerns the numerical treatment of $\Div\left(D\,\grad\dots\right)$ in equation} \eqref{heat}. 
...
\exer{8}{In the text, and the code \texttt{verifysia.m}, we used Halfar's solution to verify our numerical scheme \texttt{siaflat.m}.  Create the analogous code \texttt{verifyheat.m} to use the Green's function of the 2D heat equation \eqref{heat2D}---see the last exercise---to verify \texttt{heatadapt.m}.  You can use the high quality approximation $e^{-A^2}\approx 0$ for $|A|>10$ to choose a rectangular domain in space for which you may use $T=0$ Dirichlet boundary conditions.}
...
\exer{10}{Show, by changing variables in integrating formula \eqref{halfar}, that the volume of ice in the Halfar solution is independent of $t$.}
...
\exer{17}{Build a better ice sheet model.}

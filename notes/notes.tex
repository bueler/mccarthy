...
As show in Figure \ref{fig:domain}, in the planar (2D) case this notation becomes $z=s(t,x)$, $z=b(x)$, and $\left\{(x,z)\,:\,b(x) < z < s(t,x)\right\}$.  The set is labeled \textsf{ice} in the Figure.
...
CFL & numerical stability criterion \cite{CFL1928} & inequality FIXME \\
...
\medskip
\begin{Verbatim}[frame=single, framesep=1.5mm, xleftmargin=4mm]
...
\noindent In detail, \texttt{x}, \texttt{s}, and \texttt{snew} are one-dimensional NumPy \cite{Harrisetal2020} arrays, and \texttt{x} is assumed to hold an equally-spaced grid.  Separate ``data'' functions must define $a(x)$, $u(x)$, and $w(x)$.  This version requires the formula for $u(x)$ to produce positive numbers, but the two cases in \eqref{fd:upwind} are easily handled by the addition of a conditional statement.  The fact that \texttt{snew[0] = s[0]} is never changed is a \emph{boundary condition} compatible with the fact that the velocity is inward at the left end: \texttt{u(x[0])}$>0$ in the version here.

The rest of the code is worth a look, but there is no need to print it here.  It contains formulas for the data functions, a time-stepping loop, and calls to the \texttt{matplotlib} library to generate figures.  Various default choices are made for convenience, for example $\Delta t=1$ a and $\Delta x=1$ km, and indeed all choices are merely pedagogical.  Run the code, and view the simulation results using your favorite image viewer, as follows:
\begin{Verbatim}[xleftmargin=4mm]
$ python3 surface1d.py
evolution mode ...
  stability constant =   0.5000
  writing 50 steps to image files output/frameXXX.png
steady state mode ...
...
\caption{Initial condition (top) and the final result (bottom) of a boring solution of the SKE \eqref{skeplanar} using scheme \eqref{fd:upwind}.}
\label{fig:demo}
\end{figure}

All of this looks good and boring.  However, easily-made modifications reveal stability issues with our scheme, exposing high-spatial-frequency, rapidly-growing oscillations.  In particular, these modifications generate such exploding wiggles:
    \begin{itemize}
    ...
    \end{itemize}
This situation is understood, and in fact the condition for time-stepping stability has been known since 1928 \cite{CFL1928}.  The idea is that in the SKE \eqref{skeplanar}, information about the surface travels at the speeds $\{u_j\}$.  The maximum dimensionless Courant, Friedrichs, \& Lewy (CFL) ratio needs to be less than one so that information does not travel more than one spatial distance $\Delta x$ during the time step $\Delta t$:
\begin{equation}
\max_j \frac{|u_j| \Delta t}{\Delta x} \le 1.  \label{cfl}
\end{equation}
Our destabilizing modifications each pushed this maximum ratio above 1.

In practice, one may set the time step $\Delta t$ to assure stability.  For example, a program can, at run time, set $\Delta t = C \frac{\Delta x}{\max |u_j|}$ using $C=0.7$, so that \eqref{cfl} holds.

\subsection*{Steady state, and two horizontal dimensions}  FIXME demo: steady state of a glacier surface
%\begin{center}

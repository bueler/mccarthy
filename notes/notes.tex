...
\end{center}
However, what we really mean is that this equation is \emph{correct} in a physical sense and that it can be \emph{understood} as a glacier model.  It is very incomplete

In this section we will numerically solve the planar case \eqref{skeplanar} of the SKE for the surface elevation $s(t,x)$.  To do this we make the bold assumptions that the SMB function $a(t,x)$, \emph{and} the surface values of the velocity components, namely $u(t,x)$ and $w(t,x)$, are given by other agents.  In practice the SMB could be the output of a sub-model, such as a mass/energy process model coupled to atmosphere physics and data, and this is highly-nontrivial.  Likewise, the surface velocity components $u,w$ could be the output of a stress balance model (Section \ref{sec:continuum}), which then depends on the surface elevation $s$ itself to determine the ice domain geometry for the stress balance.\footnote{Actually, the SMB could depend on $s$ also, another form of the coupling.}  In fact the whole problem is non-trivially coupled together \cite{SchoofHewitt2013}, but we must start somewhere.

The numerical model is in a short Python program \href{https://github.com/bueler/mccarthy/blob/master/py/surface1d.py}{\texttt{py/surface1d.py}}.  (See the download discussion in Section \ref{sec:intro}.)  It generates an approximation to $s(t,x)$, over space $0<x<L$ and time $0<t<T$ intervals, and it evolves it according to \eqref{skeplanar}.  This is a \emph{time-dependent numerical solution} for the surface elevation.  We'll use this first program to consider questions of accuracy and stability, plus practicalities like debugging, verification, and visualization.
...
\end{equation}
(The precise theorem uses a finite sum and estimates the remainder, but our loose version will suffice here.)  One may replace ``$\Delta$'' by its negative or other multiples to get related forms, e.g.
    $$f(x-\Delta) = f(x) - f'(x) \Delta + \frac{1}{2} f''(x) \Delta^2 - \frac{1}{3!} f'''(x) \Delta^3 + \dots.$$

FD methods for differential equations are constructed by combining such expressions to approximate the derivatives in an equation.  The resulting algebraic equations relate unknown gridded values via mere arithmetic.  As a simple example, if we take \eqref{taylor} and subtract $f(x)$ and then divide by $\Delta$, this yields an approximation of the first derivative:
	$$\frac{f(x+\Delta) - f(x)}{\Delta} \approx f'(x).$$
The same fact can be written
	$$f'(x) = \frac{f(x+\Delta) - f(x)}{\Delta} + O(\Delta)$$
to emphasize the size of the remainder from Taylor's theorem.  Recall that $O(\Delta)$ is a quantity that is smaller than a fixed multiple of $\Delta$, asymptotically as $\Delta \to 0$.

We want to apply this FD approach to a \emph{partial} differential equation, so let us choose a time step $\Delta t>0$ and a grid spacing $\Delta x>0$.  Apply Taylor expansions for each variable to approximate the partial derivatives appearing in the SKE:
\begin{align}
...
The numerical solution will use a spatial grid of points $x_j$ with equal spacing $\Delta x$ and an integer index $j$.  It will use a temporal grid $t_n$ with (possibly unequal) spacing $\Delta t$.  A grid location is $(t_n,x_j)$.  We want to approximate a new value $s(t_{n+1},x_j)$ from current values $s(t_n,x_{\bullet})$ for several values of the spatial index.  Actually, to be a bit more careful, we distinguish between the exact solution value $s(t_n,x_j)$, which evaluates the generally--unknown solution of the continuum SKE \eqref{skeplanar} at some grid point $(t_n,x_j)$, and our numerical approximation of this value, denoted by $s_j^n$.  This is \emph{not} the exactly correct value $s(t_n,x_j)$, but we hope that $s_j^n \approx s(t_n,x_j)$.  In any case, we will actually compute the values $s_j^n$.

Furthermore we will significantly declutter the grid notation by writing $s_j$ for $s_j^n$ and $\snew_j$ for $s_j^{n+1}$.  We these conventions, into \eqref{skeplanar} we make the replacements \eqref{fd:partials}
...
and so the SKE becomes discrete:
\begin{equation*}
...
\end{equation}
Note that this scheme uses 3 values at the current time to determine one value at the new time, and a simple \emph{stencil} diagram captures this meaning (Figure \ref{fig:stencils}, left), and it helps in remembering the standard name \emph{forward-time, centered-space} for the scheme.

\begin{figure}[ht]
\input{tikz/ftcs.tex} \quad \input{tikz/upwind.tex}
\caption{To compute $\snew_j$ (circle), scheme \eqref{fd:ftcs} (left) uses 3 gridded values at the current time $t_n$ (solid), while the upwind scheme \eqref{fd:upwind} (right; for $u_j>0$) uses just 2, but depending on the flow direction.}
\label{fig:stencils}
\end{figure}

However, if we implement scheme \eqref{fd:ftcs} in code then the simulation will exhibit wiggles which do not go away.  The well-known \cite{MortonMayers2005}, if surprising, remedy is to revert to a less-accurate FD choice in space, one which is also asymmetric.  For the FD approximation of $\partial s/\partial x$ we take
...
otherwise ($u_j \le 0$).  These formulas have only first-order accuracy, versus the formula in \eqref{fd:partials}, but this \emph{upwind} differencing choice exhibits (conditional) stability and avoidance of wiggles.  Written comparably to \eqref{fd:ftcs}, the upwind scheme is
\begin{equation}
...
\end{equation}
as stenciled in Figure \ref{fig:stencils}, right.

\subsection*{Implementation}  We can now look at the code.  This upwind scheme for the SKE becomes a short Python function:
...
\noindent In detail, \texttt{x}, \texttt{s}, \texttt{snew} are one-dimensional NumPy \cite{Harrisetal2020} arrays, where \texttt{x} is assumed to be an equally-spaced grid, and \texttt{dt} is a provided time step.  Separate functions must define $a(x)$, $u(x)$, and $w(x)$.  Note that \texttt{snew[0] = s[0]} is never changed; this is a boundary condition.

FIXME demo: evolution of glacier surface
...
the condition for time-stepping stability is known; information about the surface travels at speed $u$; from grid picture, the upwind scheme can only be stable if
...
rearranged, this is the \alert{CFL condition} on the time step:

...
The upper surface evolves in part through the \emph{surface mass balance rate} (SMB), denoted $a(t,x,y)$, or $a(t,x)$ in the planar case.  The SMB is also known as the \emph{climatic} mass balance \cite{Cogleyetal2011}.  By definition this is the balance between accumulating snow and the loss of melt water, by melt of the snow, firn, or ice, through runoff, at the upper surface.\footnote{These notes only consider simulations of land-based glaciers, without floating portions, but ice shelves are also subject to a \emph{basal} mass balance rate.}  Our elevations are measured in meters, and time in seconds, and so the SMB function $a$ is measured in ice-equivalent meters per second.

The upper surface evolves according to the \emph{surface kinematical equation} (SKE), which can be written in the planar case as
\begin{equation}
\frac{\partial s}{\partial t} = a + w -u \frac{\partial s}{\partial x}. \label{ske}
\end{equation}
Recalling our notation $\bu=\left<u,w\right>$ for the planar ice velocity, the variables $u$ and $w$ in \eqref{eq:ske} are the upper surface values of the velocity, i.e.~$u=u(t,x,s(t,x))$ because $z=s(t,x)$ at the upper surface, and similarly for $w$.

FIXME the ice surface moves up and down according to
\begin{itemize}

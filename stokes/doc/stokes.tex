...
\begin{tikzpicture}[node distance=3.8cm,auto,>=latex']
    ...
    \node [tool] (physics) [above left of=flow, xshift=1cm, yshift=-0.5cm] {\texttt{physics.py}};
    \node [tool] (meshactions) [above right of=flow, xshift=-1cm, yshift=-0.5cm] {\texttt{meshactions.py}};
...
\medskip
\caption{The tools are used in the given order (solid arrows), with input/output file formats as shown.  Dashed arrows show Python \texttt{import}s.}
\end{figure}

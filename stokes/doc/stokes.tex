...
\usepackage[total={6.3in,9.7in},top=0.9in,left=1.1in]{geometry}
...
We start by stating the Stokes model with glacier-suitable boundary conditions, and then we construct slab-on-a-slope solutions both for verification and to clarify the boundary conditions.  Then we derive the ``weak form,'' as explained below.  An overview of finite element (FE) methods \cite{Elmanetal2014}, which are based on such weak forms, follows.  Our particular FE method uses triangular elements for an arbitrary planar region and stable mixed elements; these ideas are only briefly explained.

The main goal of this appendix comes next, namely a numerical velocity and pressure solution to a 2D glacier with a step in the bedrock (Figure \ref{fig:stepflowlin}).  This uses four advanced open source tools/libraries:
\begin{itemize}
...
Recall the Glen-Stokes model in equations (3), (4), (5) from the notes.  (The model is also described in \cite{GreveBlatter2009,JouvetRappaz2011}.)  The model applies on a 3D or 2D domain $\Omega$ according to the context.  Allowing any Glen exponent $n\ge 1$, the equations are:
\begin{align}
...
\end{equation}
so equation \eqref{forcebalance} is simply $-\Div \sigma = \rho \bg$.  One may derive from \eqref{cauchystress} that $p = -\frac{1}{3} \trace(\sigma)$ thus that the pressure is the negative of the average normal stress.  By definition $\Div\tau$ in \eqref{forcebalance} is a vector with components which are the divergences of the rows:
...
\end{equation}
Here $B_n = (A_n)^{-1/n}$ is the $n$-dependent ice hardness in units $\text{Pa}\,\text{s}^{1/n}$.  From \eqref{viscflowlaw} we can eliminate $\tau$ and rewrite equations \eqref{incompressible}, \eqref{forcebalance} in terms of velocity and pressure only:
\begin{align}
...
The domain $\Omega$ must have a smooth-enough boundary to apply the boundary conditions but it is otherwise general.  However, certain glacier-suitable boundary conditions are used in this document and in the code below.  To explain these, consider the numerical solution shown in Figure \ref{fig:stepflowlin}.  We assume that the base, top, inflow, and outflow boundary surfaces are all well-defined.  On the base we require no slip:
\begin{align}

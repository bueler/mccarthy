...
%./gendomain.py -hmesh 300.0 -refine_corner 2 -o low.geo
%gmsh -2 low.geo
%./flow.py low.msh -s_mat_type aij -s_ksp_view_mat :low.m:ascii_matlab
% then spy() in Matlab
...
For example, solving a time-independent Stokes problem for $(\bu,p)$ looks like this:
...
\medskip
\noindent Note that codes \texttt{gendomain.py} and \texttt{flow.py} can also provide help by the \texttt{-h} option.

In a time-stepping run one supplies the time step in days (\texttt{-deltat}) and the number of steps (\texttt{-m}) as arguments to \texttt{flow.py}.  For example, at the final time the following run generates the result shown in Figure \ref{fig:glacier}:
